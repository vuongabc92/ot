        ...
        <tr>
            ...
            <td>
                @if( ! check_file($image_path . $post->image))
                    <span class="default-image default-post-img _r5 fa fa-image"></span>
                @else
                    <img src="{{ asset($image_path . $post->image) }}" class="post-image img-responsive img-rounded"/>
                @endif
            </td>
            <td>{{ $post->name }}</td>
            <td>
                @if($post->is_active)
                <a class="label label-success _fs11 _r2" href="{{ route('backend_post_active', $post->id) }}">{{ _t('backend_common_active') }}</a>
                @else
                <a class="label label-danger _fs11 _r2" href="{{ route('backend_post_active', $post->id) }}">{{ _t('backend_common_disable') }}</a>
                @endif
            </td>
            ...
            <td>
                ...
                <a href="{{ route('backend_post_delete', ['id' => $post->id, 'token' => csrf_token()]) }}" class="btn btn-danger btn-sm _r2" onclick="return confirm('{{ _t('backend_post_delete_one') }}')">{{ _t('backend_common_delete') }}</a>
            </td>
        </tr>
        @endforeach
    </tbody>
